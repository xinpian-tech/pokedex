...
struct Inst {
    ...
    // frm field: inst[14:12]
    FrmField rm() const {
        return FrmField { .value = uint8_t((data >> 12) & 0x07) };
    }
...
struct CInst {
    ...
    FRegIdx fd()     const { return freg_from_idx((data >> 7) & 0x1f); }
    FRegIdx cr_fs2() const { return freg_from_idx((data >> 2) & 0x1f); }
    ...
    FRegIdx fd_c()  const { return freg_from_idx(((data >> 2) & 0x7) + 8); }
    FRegIdx fs2_c() const { return freg_from_idx(((data >> 2) & 0x7) + 8); }
